...
### Step 7: Update all 31 `config/examples/launch/*.yaml`
...
| `config/examples/launch/**/*.yaml` (31 files) | Add `eval` section (inloop, 60 tasks, 3 families) |
...
4. `olmix launch run --config config/examples/launch/data_proportions/mix_baseline.yaml --dry-run` — launch with inloop eval config (no Beaker needed)
